        ...
        if (mysqli_query($con, $query)) 
            {
               ...
               ?>
               <!--<br><a href="". $_SERVER["SCRIPT_NAME"] ."?action=getalltasks&username=<?php echo $UserName ?>"><BUTTON style="height:100px;width:200px;color:green;font-size:20px;"><<< Back to Tasks</BUTTON></a> -->
               <?php
            }
     }  
}
...
if($Action == "getalltasks" || strcmp($Action,"completetask")==0 || $Action == "" || strcmp($Action,"canceltask")==0) {
    ...
    if($row_cnt > 0){
?>
<Table border='1' id='tblShowAllTasks' width='60%' align='left'>
    <th><h1><?php echo $UserName ?> Tasks</h1></th>
<?php
    if (!$result = $con->query($sql)) {
        die ('There was an error running query[' . $con->error  . ']');
    }
    ...
    ?>
        <tr><td class="<?php echo $TDstyle ?>">
            <form action='webTask_history.php' method='post'><input type='hidden' id='cookie' name='cookie' value='reset'>
            <input type="hidden" name="username" value="<?php echo $UserName ?>"><input type='submit' class="btn btn-success" style="font-size : 20px; width: 200px; height:70px;" value='TASK HISTORY'></form></td><td class="<?php echo $TDstyle ?>" >
