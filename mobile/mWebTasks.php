...
{
header("Location: mLogin.php?status=loggedout");
die();
}
...
<div class="toast d-none" role="alert" id="toastAlert" aria-live="assertive" aria-atomic="true">
  <div class="toast-header">
    <img src="../images/alert.png" class="rounded mr-2" alt="...">
...
//echo "theme=".$TDstyle;
?>
<body style='background-color:<?php echo $BackGroundColor ?>>;'  onload='countFilters();'>
<?php
include "SideNav.css";
echo file_get_contents( "SideNav.php"); // get the contents, and echo it out.
...
$UnitNameForInfo = "";
echo "<img src='../images/menu.gif' style='width:20vw;height:*; max-width:16vw;margin-top:30px;' onclick='openNav()'";
...
?>
<tr><td class="<?php echo $TDstyle ?>"></td>
<td class="<?php echo $TDstyle ?>"><form action='webTask_history.php' method='post'><button type="button" class="btn btn-success" style="font-size : 20px; width: 200px; height:70px;">TASK HISTORY</button><input type='hidden' id='cookie' name='cookie' value='reset'>
...
$effectiveDate=getNextDueDate($Rotation);
    ...
    if(isset($_POST['unit_id'])){$UnitID = $_POST['unit_id'];}
        ...
        if (mysqli_query($con, $sql)) {
         ...
         ?>
         <!--<a href="". $_SERVER["SCRIPT_NAME"] ."?action=getalltasks&username=<?php echo $UserName ?>"><BUTTON style="height:100px;width:200px;color:green;font-size:20px;"><<< Back to Tasks</BUTTON></a>-->
         <?php
        } else {
...
if($Action == "getalltasks" || strcmp($Action,"completetask")==0 || $Action == "" || strcmp($Action,"canceltask")==0) {
    ...
    if($row_cnt > 0){
?>
<Table border='1' id='tblShowAllTasks' width='60%' align='left' style="background-color:blue;">
    <th><img src="../images/menu.gif" style="width:20vw;height:*; max-width:16vw;margin-top:30px;" onclick="openNav()" style="display:inline-block;margin-left:0px;"></th><th><h1 style="color:white;"><?php echo $UserName ?> Tasks</h1></th>
<?php
    if (!$result = $con->query($sql)) {
        die ('There was an error running query[' . $con->error  . ']');
    }
...
$arFilterSizes = array();
     while ($row = $result->fetch_assoc()) 
        ...
        ?>
            ...
            <tr><td class="<?php echo $TDstyle ?>">Unit name:</td><td class="<?php echo $TDstyle ?>">
                    <a href="<?php echo  $_SERVER["SCRIPT_NAME"]  ?>?username=<?php echo $UserName ?>&unit_nameforinfo=<?php echo $row["unit_name"] ?>&unitid=<?php echo $row["unit_id"] ?>"><?php echo $row["unit_name"] ?></a>
		<?php
              
                     if(strcmp($row["unit_name"], $UnitNameForInfo) == 0)
						{
							 echo "<br><textarea style='background-color:green;color:white;font-size: 2.5vw;'  rows='9' cols='35' id='txtinfo'". $row["task_id"] ." ".$display .">"
         ...
         ?>
                    ...
                    <tr>
                     <td class="<?php echo $TDstyle ?>">Location:</td><td class="<?php echo $TDstyle ?>"><?php echo $row["unit_location"] ?></td></tr>
                     <tr><td class="<?php echo $TDstyle ?>">Filters needed:</td><td class="<?php echo $TDstyle ?>"><?php echo $row["filters_needed"] ?></td></tr>
                    
                    <form action="<?php echo  $_SERVER["SCRIPT_NAME"]  ?>" method="POST" name="frmName<?php echo $row["task_id"] ?>" id="frmCompleteTask<?php echo $row["task_id"] ?>">
                    <tr><td class="<?php echo $TDstyle ?>">Filter type:</td><td class="<?php echo $TDstyle ?>" align="center"><div style="width: 100%; height: 100%;">
                    <select  id="slctFilterType<?php echo $row["task_id"] ?>" name="filter_type" class="ftype" style="display: flex;font-size: 2vw;margin:auto;" onchange="setFilterTypes('<?php echo $row["task_id"] ?>');">
            ...
            ?>
                    ...
                    <td class="<?php echo $TDstyle ?>" align="center"><div style="width: 100%; height: 100%;">
                     <form action="<?php echo  $_SERVER["SCRIPT_NAME"]  ?>" method="post" id="<?php echo "frmCancel".$row["task_id"] ?>">
                    ...
                    <img src="../images/cancel.png" style="width:35px;height:auto;margin-top:10px;" onclick="myConfirmBox('Do you wish to cancel this task?', 'frmCancel<?php echo $row["task_id"] ?>')"></td>
                     </form>
                     </div></td>
                
					<td class="<?php echo $TDstyle ?>" align="center"><img id="img<?php echo $row["task_id"] ?>" src="../images/dwcheckyes.png" onclick="CompleteFilters(<?php echo $row["task_id"] ?>);" style="width:30px;height:30px;"></td>
                ...
                </td></tr><tr style="width:100%;height:50px;color:black;"></tr>
                <?php
                $n=$n+1;
        }
        //FOR SUBMIT ALL TASKS BUTTON 
        echo "<tr><form id='frmCompleteAll' action='". $_SERVER["SCRIPT_NAME"] ."' method='post'>";
        ...
        $keys = array_keys($arAllData);
            for($i = 0; $i < count($arAllData); $i++) {
                ...
                echo "<input type='hidden' id='hdnFtype".$thisTaskID."' name='filter_type[]' value='select'>";
                     ...
                     echo"</form></div><tr><td></td><td style='text-align:center;'>";
                    ?> <button style="background-color:blue;color:white;width:150px;height:60px;" onclick="ConfirmBox('Complete all tasks?');" id='btnCompleteAllTasks' title="Get all filters selected above before clicking" disabled>Complete All Tasks</button></td> <?php
         }
        else
        {
            echo "<tr><td class='<?php echo $TDstyle ?>'>
            <td class='<?php echo $TDstyle ?>' align='center'><div style='width: 100%; height: 100%; color: white; font-size:20px;font-weight: bold;background-color:#3FBF3F;'>".$UserName. " you have no tasks</div>
            </td>
            <td class='<?php echo $TDstyle ?>'></td></tr><tr style='background-color:black;height:20px;width:100%'></tr>";
        }
    }
    ?>
        <tr><td class="<?php echo $TDstyle ?>"></td>
    ...
    <tr><th><div id="divFiltersNeeded" style="float:left;display:flex;color:white;"></div></th></tr>
    <tr><td>
    <div id='divFiltersCounted' style='color:black;height:auto;float:left;display:flex;flex-direction:row;"'>
<div id='divAmount' style="width: 50%;overflow: auto;text-align: right;color:black;"></div>
<div id='divSize' style="width: 50%;overflow: auto;color:black;"></div>

</div> 
</td>
</tr><tr>
<td class="<?php echo $TDstyle ?>" >
            <?php if($row_cnt > 0){ echo "<form action = '../PrintTasks.php' method='POST'><input type='submit' class='btn btn-success' style='font-size : 20px; width: 200px; height:70px;'  value='PRINT...'>";} ?>
            </td></tr>
</table>
