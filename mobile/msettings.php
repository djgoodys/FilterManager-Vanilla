            ...
            if (mysqli_query($con, $query)) 
               {
                ...
                {
                    echo "Error updating company log image: " . mysqli_error($con);
                }
...
<a class="dropdown-item" style="font-family:Franklin Gothic Medium;"  href="<?php echo $_SERVER["SCRIPT_NAME"] ?>?font_family=Franklin Gothic Medium">Franklin Gothic Medium</a>
        ...
        <td style="width:33%;">
        <form action="<?php echo $_SERVER["SCRIPT_NAME"] ?>" method="post" target="iframe2">
         <select class="form-select users-style" id='selTheme' class="form-select form-select-lg mb-3" aria-label=".form-select-lg example" name='theme' onchange='this.form.submit();'>
         <option  value='select_theme'>Theme</option>
         <option  value='Dark-tdPlain' <?php if($Theme =="Dark-tdPlain"){echo " selected";} ?>>dark</option>
         <option  value='Light-tdPlain'  <?php if($Theme =="Light-tdPlain"){echo " selected";} ?>>light</option></select>
         </form>
