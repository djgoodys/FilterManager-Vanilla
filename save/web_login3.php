...
//echo "servername=".$_SESSION["server_name"]."<BR>";
if(isset($_COOKIE["cookie_username"])) {
  if(strLen($_COOKIE["cookie_username"]) > 0)
    {
      $UserNamePassed="true";
      $UserName=$_COOKIE["cookie_username"];
      $query = "SELECT * FROM users WHERE user_name = '".$UserName."';";
      $result = $UsersCon -> query($query);
...
<?php
 ...
 //FIND OUT IF ANY USERS HAVE BEEN CREATED
 $query="SELECT * FROM users WHERE admin = 'on';";
 $result = $UsersCon -> query($query);
 //$row = $result -> fetch_assoc();
...
if($UserNamePassed == "true"){
  ...
  $arUnits = array();
 //echo $_SESSION["server_name"]." ".$_SESSION["database_username"]." " .$_SESSION["database_password"]." " .$_SESSION["database_name"];
...
<?php
     ...
     ?><input type="hidden" id="txtLoggedIn" value="<?php echo $UserNamePassed; ?>"> 
